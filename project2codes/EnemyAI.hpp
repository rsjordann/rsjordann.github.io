	...
	int hitsRemainingPlayer[])
{
	...
	int shipLocProbability[BOARD_LENGTH][BOARD_LENGTH]; {};
	...
	do
	{
		...
		// biggest ship.
		calculateProbabilityMatrix(playersBoard, shipLocProbability,
			hitsRemainingPlayer);

		// Pick the next shot randomly based on probable locations.
		enemyPick = randomFromMaxProbability(shipLocProbability);
...
int EnemyAI::randomFromMaxProbability (const int matrix[][BOARD_LENGTH])
{
	...
	vector<int> highestProbLocations; // a variable-size "array"
	...
	for (rowIndex = BOARD_LENGTH - 1; rowIndex >= 0; --rowIndex)
	{
		...
		{
			// The new value is at least as big as the previous
			if (matrix[rowIndex][colIndex] >= max)
			{
				// The new value is bigger than all the previous
				if (matrix[rowIndex][colIndex] > max)
				{
					// Empty the old max values.
					highestProbLocations.clear();
					max = matrix[rowIndex][colIndex];
				}

				// Store the latest max value.
...
int EnemyAI::randomFromProbabilityMatrix (const int matrix[][BOARD_LENGTH])
{
	...
	// than the total.
	cerr << "ERROR: Invalid Probability.\n";
